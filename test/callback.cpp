...
struct callback_cmd
{
    ...
    void parse(F f)
    {
        auto cb = [this](auto& c, const auto&, const args::argument&) {
            for(int i=0;i<c;i++) final_name += name;
        };
...
PROVE_CASE()
{
    ...
    PROVE_CHECK(cmd.name == "hello");
    PROVE_CHECK(cmd.final_name == "hello");
}
...
PROVE_CASE()
{
    ...
    PROVE_CHECK(cmd.count == 1);
    PROVE_CHECK(cmd.name == "hello");
    PROVE_CHECK(cmd.final_name.empty());
}
...
PROVE_CASE()
{
    ...
    PROVE_CHECK(cmd.count == 5);
    PROVE_CHECK(cmd.name == "hello");
    PROVE_CHECK(cmd.final_out.empty());
}
